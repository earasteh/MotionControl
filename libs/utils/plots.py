import matplotlib.pyplot as plt
import numpy as np
import pandas as pd
import os
from libs.utils.env import world

"""
This file cleans and labels the data and then plots all the results
"""


# Variables to track and plot:
# U, V, wz, wFL, wFR, wRL, wRR, yaw, x, y = self.state
# U_dot, V_dot, wz_dot, wFL_dot, wFR_dot, wRL_dot, wRR_dot, yaw_dot, x_dot, y_dot = state_dot
# fFLx, fFRx, fRLx, fRRx, fFLy, fFRy, fRLy, fRRy, fFLz, fFRz, fRLz, fRRz, sFL, sFR, sRL, sRR, fFLxt, fFLyt = outputs

def data_cleaning(DataLog):
    DataLog_nz = DataLog[~np.all(DataLog == 0, axis=1)]  # only plot the rows that are not zeros
    DataLog_pd = pd.DataFrame(DataLog_nz, columns=['time',
                                                   'U', 'V', 'wz', 'wFL', 'wFR', 'wRL', 'wRR', 'yaw', 'x', 'y',
                                                   'U_dot', 'V_dot', 'wz_dot', 'wFL_dot', 'wFR_dot', 'wRL_dot',
                                                   'wRR_dot', 'yaw_dot', 'x_dot', 'y_dot',
                                                   'delta', 'tau_FL', 'tau_FR', 'tau_RL', 'tau_RR',
                                                   'Fx_FL', 'Fx_FR', 'Fx_RL', 'Fx_RR',
                                                   'Fy_FL', 'Fy_FR', 'Fy_RL', 'Fy_RR',
                                                   'Fz_FL', 'Fz_FR', 'Fz_RL', 'Fz_RR',
                                                   'sFL', 'sFR', 'sRL', 'sRR', 'Fxt_FL', 'Fyt_FL', 'crosstrack',
                                                   'x0-x', 'x0-y', 'x0-yaw', 'x0-vx', 'x0-vy', 'x0-omega', 'x0-tau',
                                                   'x0-delta',
                                                   'xN-x', 'xN-y', 'xN-yaw', 'xN-vx', 'xN-vy', 'xN-omega', 'xN-tau',
                                                   'xN-delta', 'flag']
                              )
    if not os.path.exists('results'):
        os.makedirs('results')
    try:
        DataLog_pd.to_csv('results/Results.csv')
    except FileNotFoundError:
        fp = open('results/Results.csv', 'x')
        fp.close()
        DataLog_pd.to_csv('results/Results.csv')

    return DataLog_pd


def plot_results(DataLog_pd, WhichPlots, figsize_input=None):
    if 'vx' in WhichPlots:
        plt.figure(figsize=figsize_input)
        fig_name = 'Forward Velocity'
        plt.title(fig_name)
        plt.plot(DataLog_pd['time'], DataLog_pd['U'])
        plt.grid()
        plt.xlabel('Time (Sec.)')
        plt.ylabel('Forward Velocity (m/s)')
        plt.savefig('results/' + fig_name + '.png', dpi=150)

    if 'vy' in WhichPlots:
        plt.figure(figsize=figsize_input)
        fig_name = 'Lateral Velocity'
        plt.title(fig_name)
        plt.plot(DataLog_pd['time'], DataLog_pd['V'])
        plt.grid()
        plt.xlabel('Time (Sec.)')
        plt.ylabel('Lateral Velocity (m/s)')
        plt.savefig('results/' + fig_name + '.png', dpi=150)

    if 'yaw rate' in WhichPlots:
        plt.figure(figsize=figsize_input)
        fig_name = 'Yaw rate'
        plt.title(fig_name)
        plt.plot(DataLog_pd['time'], DataLog_pd['yaw_dot'])
        plt.grid()
        plt.xlabel('Time (Sec.)')
        plt.ylabel('Yaw rate (rad/sec)')
        plt.savefig('results/' + fig_name + '.png', dpi=150)

    if 'latac' in WhichPlots:
        plt.figure(figsize=figsize_input)
        fig_name = 'Lateral Acceleration'
        plt.title(fig_name)
        plt.plot(DataLog_pd['time'], DataLog_pd['V_dot'])
        plt.grid()
        plt.xlabel('Time (Sec.)')
        plt.ylabel('Lateral Acceleration (m/s^2)')
        plt.savefig('results/' + fig_name + '.png', dpi=150)

    # plt.figure()
    # plt.title('Longitudinal Tire Forces')
    # plt.plot(DataLog_pd['time'], DataLog_pd['Fx_FL'])
    # plt.plot(DataLog_pd['time'], DataLog_pd['Fx_FR'])
    # plt.plot(DataLog_pd['time'], DataLog_pd['Fx_RL'])
    # plt.plot(DataLog_pd['time'], DataLog_pd['Fx_RR'])
    # plt.legend(['FL', 'FR', 'RL', 'RR'])
    # plt.xlabel('Time (Sec.)')
    # plt.ylabel('Force (N)')

    if 'Fy' in WhichPlots:
        plt.figure(figsize=figsize_input)
        fig_name = 'Lateral Tire Forces'
        plt.title(fig_name)
        plt.plot(DataLog_pd['time'], DataLog_pd['Fy_FL'])
        plt.plot(DataLog_pd['time'], DataLog_pd['Fy_FR'])
        plt.plot(DataLog_pd['time'], DataLog_pd['Fy_RL'])
        plt.plot(DataLog_pd['time'], DataLog_pd['Fy_RR'])
        plt.grid()
        plt.legend(['FL', 'FR', 'RL', 'RR'])
        plt.xlabel('Time (Sec.)')
        plt.ylabel('Force (N)')
        plt.savefig('results/' + fig_name + '.png', dpi=150)

    if 'FN' in WhichPlots:
        plt.figure(figsize=figsize_input)
        fig_name = 'Normal Tire Forces'
        plt.title(fig_name)
        plt.plot(DataLog_pd['time'], DataLog_pd['Fz_FL'])
        plt.plot(DataLog_pd['time'], DataLog_pd['Fz_FR'])
        plt.plot(DataLog_pd['time'], DataLog_pd['Fz_RL'])
        plt.plot(DataLog_pd['time'], DataLog_pd['Fz_RR'])
        plt.grid()
        plt.legend(['FL', 'FR', 'RL', 'RR'])
        plt.xlabel('Time (Sec.)')
        plt.ylabel('Force (N)')
        plt.savefig('results/' + fig_name + '.png', dpi=150)

    if 'tau' in WhichPlots:
        plt.figure()
        plt.title('Torque at each wheel')
        plt.subplot(2, 2, 1)
        plt.plot(DataLog_pd['time'], DataLog_pd['tau_FL'])
        plt.legend(['FL'])
        plt.xlabel('Time (Sec.)')
        plt.ylabel('Wheel Torque (N.m)')
        plt.subplot(2, 2, 2)
        plt.plot(DataLog_pd['time'], DataLog_pd['tau_FR'])
        plt.legend(['FR'])
        plt.xlabel('Time (Sec.)')
        plt.ylabel('Wheel Torque (N.m)')
        plt.subplot(2, 2, 3)
        plt.plot(DataLog_pd['time'], DataLog_pd['tau_RL'])
        plt.legend(['RL'])
        plt.xlabel('Time (Sec.)')
        plt.ylabel('Wheel Torque (N.m)')
        plt.subplot(2, 2, 4)
        plt.plot(DataLog_pd['time'], DataLog_pd['tau_RR'])
        plt.legend(['RR'])
        plt.xlabel('Time (Sec.)')
        plt.ylabel('Wheel Torque (N.m)')

    if 'omega-wheel' in WhichPlots:
        plt.figure()
        plt.title('Angular velocity at each wheel')
        plt.subplot(2, 2, 1)
        plt.plot(DataLog_pd['time'], DataLog_pd['wFL'])
        plt.legend(['FL'])
        plt.xlabel('Time (Sec.)')
        plt.ylabel('Angular Vel. (rad/Sec)')
        plt.subplot(2, 2, 2)
        plt.plot(DataLog_pd['time'], DataLog_pd['wFR'])
        plt.legend(['FR'])
        plt.xlabel('Time (Sec.)')
        plt.ylabel('Angular Vel. (rad/Sec)')
        plt.subplot(2, 2, 3)
        plt.plot(DataLog_pd['time'], DataLog_pd['wRL'])
        plt.legend(['RL'])
        plt.xlabel('Time (Sec.)')
        plt.ylabel('Angular Vel. (rad/Sec)')
        plt.subplot(2, 2, 4)
        plt.plot(DataLog_pd['time'], DataLog_pd['wRR'])
        plt.legend(['RR'])
        plt.xlabel('Time (Sec.)')
        plt.ylabel('Angular Vel. (rad/Sec)')

    if 'delta' in WhichPlots:
        plt.figure(figsize=figsize_input)
        fig_name = 'Steering Angle'
        plt.title(fig_name)
        plt.plot(DataLog_pd['time'], DataLog_pd['delta'] * 180 / np.pi)
        plt.grid()
        plt.xlabel('Time (Sec.)')
        plt.ylabel('Steering angle (deg)')
        plt.savefig('results/' + fig_name + '.png', dpi=150)

    if 'crosstrack' in WhichPlots:
        fig, ax = plt.subplots(figsize=figsize_input)
        ax.set_xlim(0.1, 5)
        fig_name = 'Cross Track Error'
        plt.title(fig_name)
        plt.plot(DataLog_pd['time'], DataLog_pd['crosstrack'])
        plt.grid()
        plt.xlabel('Time (Sec.)')
        plt.ylabel('Cross Track Error (m)')
        plt.savefig('results/' + fig_name + '.png', dpi=150)

    if 'slip' in WhichPlots:
        plt.figure(figsize=figsize_input)
        fig_name = 'Combined slip'
        plt.title(fig_name)
        plt.subplot(2, 2, 1)
        plt.plot(DataLog_pd['time'], DataLog_pd['sFL'])
        plt.grid()
        plt.legend(['FL'])
        plt.xlabel('Time (Sec.)')
        plt.ylabel('Slip')
        plt.subplot(2, 2, 2)
        plt.plot(DataLog_pd['time'], DataLog_pd['sFR'])
        plt.grid()
        plt.legend(['FR'])
        plt.xlabel('Time (Sec.)')
        plt.ylabel('Slip')
        plt.subplot(2, 2, 3)
        plt.plot(DataLog_pd['time'], DataLog_pd['sRL'])
        plt.grid()
        plt.legend(['RL'])
        plt.xlabel('Time (Sec.)')
        plt.ylabel('Slip')
        plt.subplot(2, 2, 4)
        plt.plot(DataLog_pd['time'], DataLog_pd['sRR'])
        plt.grid()
        plt.legend(['RR'])
        plt.xlabel('Time (Sec.)')
        plt.ylabel('Slip')
        plt.savefig('results/' + fig_name + '.png', dpi=150)

    if 'traj' in WhichPlots:
        fig, ax = plt.subplots(figsize=(10, 6))
        plt.grid()
        fig_name = 'Trajectory of the vehicle'
        plt.title(fig_name)
        plt.plot(DataLog_pd['x'], DataLog_pd['y'], color='green')
        plt.plot(world.xm, world.ym, 'k--')
        plt.fill(world.obstacle_x, world.obstacle_y, color='red', zorder=2)
        # ax.set_xlim(-10, 100)
        # ax.set_ylim(-5, 25)
        ax.set_aspect('equal')
        ax.legend(['Generated Path', 'Ref'])
        plt.xlabel('X (meters)')
        plt.ylabel('Y (meters)')
        plt.savefig('results/' + fig_name + '.png', dpi=150)

    if 'MPC-flag' in WhichPlots:
        plt.figure(figsize=figsize_input)
        fig_name = 'Flag'
        plt.title(fig_name)
        plt.plot(DataLog_pd['time'], DataLog_pd['flag'])
        plt.grid()
        plt.xlabel('Time (Sec.)')
        plt.ylabel('MPC Flag')
        plt.savefig('results/' + fig_name + '.png', dpi=150)

    if 'MPC-x0-states' in WhichPlots:
        # 'x0-x', 'x0-y', 'x0-yaw', 'x0-vx', 'x0-vy', 'x0-omega', 'x0-tau',
        # 'x0-delta'
        plt.figure(figsize=(18, 8))
        plt.subplot(2, 3, 1)
        plt.suptitle('MPC initial state propagation', fontsize=16)
        plt.plot(DataLog_pd['time'], DataLog_pd['x0-x'])
        plt.plot(DataLog_pd['time'], DataLog_pd['flag'], color='red')
        plt.xlabel('Time (Sec.)')
        plt.ylabel('X (m)')
        plt.grid()
        plt.subplot(2, 3, 2)
        plt.plot(DataLog_pd['time'], DataLog_pd['x0-y'])
        plt.plot(DataLog_pd['time'], DataLog_pd['flag'], color='red')
        plt.xlabel('Time (Sec.)')
        plt.ylabel('Y (m)')
        plt.grid()
        plt.subplot(2, 3, 3)
        plt.plot(DataLog_pd['x0-x'], DataLog_pd['x0-y'])
        plt.plot(DataLog_pd['x'], DataLog_pd['y'])
        plt.legend(['MPC', 'Real'])
        plt.xlabel('X (m)')
        plt.ylabel('Y (m)')
        plt.grid()
        plt.subplot(2, 3, 4)
        plt.plot(DataLog_pd['time'], DataLog_pd['x0-yaw'] * 180 / np.pi)
        plt.plot(DataLog_pd['time'], DataLog_pd['flag'], color='red')
        plt.xlabel('Time (Sec.)')
        plt.ylabel('Yaw (deg.)')
        plt.grid()
        plt.subplot(2, 3, 5)
        plt.plot(DataLog_pd['time'], DataLog_pd['x0-vx'])
        plt.plot(DataLog_pd['time'], DataLog_pd['x0-vy'])
        plt.plot(DataLog_pd['time'], DataLog_pd['flag'], color='red')
        plt.xlabel('Time (Sec.)')
        plt.ylabel('V (m/s)')
        plt.legend(['$V_x$', '$V_y$'])
        plt.grid()
        plt.subplot(2, 3, 6)
        plt.plot(DataLog_pd['time'], DataLog_pd['x0-omega'])
        plt.xlabel('Time (Sec.)')
        plt.ylabel('$\omega$ (rad/s)')
        plt.grid()

        if 'MPC-inputs' in WhichPlots:
            # 'x0-x', 'x0-y', 'x0-yaw', 'x0-vx', 'x0-vy', 'x0-omega', 'x0-tau',
            # 'x0-delta'
            plt.figure()
            plt.subplot(2, 1, 1)
            plt.suptitle('MPC input propagation', fontsize=16)
            plt.plot(DataLog_pd['time'], DataLog_pd['x0-delta'] * 180 / np.pi)
            plt.plot(DataLog_pd['time'], DataLog_pd['flag'], color='red')
            plt.grid()
            plt.xlabel('Time (Sec.)')
            plt.ylabel('$\delta$ (deg.)')
            plt.subplot(2, 1, 2)
            plt.plot(DataLog_pd['time'], DataLog_pd['x0-tau'])
            plt.plot(DataLog_pd['time'], DataLog_pd['flag'], color='red')
            plt.grid()
            plt.xlabel('Time (Sec.)')
            plt.ylabel('$ \\tau $ (N.m)')

        plt.savefig('results/' + fig_name + '.png', dpi=150)

    plt.show()


def main():
    curr_path = os.getcwd()
    os.chdir('../..')
    DataLog_pd = pd.read_csv('results/Results.csv')
    plot_results(DataLog_pd, WhichPlots=('crosstrack', 'traj', 'delta', 'flag', 'MPC-x0-states', 'MPC-inputs'))


if __name__ == "__main__":
    main()
